...
Main.prototype = {
	initialize: function(e) {
		Main._canvas = window.document.getElementById("canvas");
		Main._canvas.width = 1000;
		Main._canvas.height = 600;
		Main._stage = new createjs.Stage(Main._canvas);
		Main._stage.autoClear = false;
		...
		this._onResize(null);
		Main._stage.update();
		Main._stage.addEventListener("stagemousedown",$bind(this,this._onDown));
	}
	,_initMoji: function() {
		var list = ["The quick_brown fox_jumps over_the lazy dog","Kitasenju_Design is a_Adachi based_design unit"];
		var s = list[this._index % list.length];
		this._index++;
		var allPoints = [];
		var font = new net.badimon.five3D.typography.HelveticaMedium();
		var xx = 60;
		var yy = -20;
		var _g1 = 0;
		var _g = s.length;
		while(_g1 < _g) {
			var i = _g1++;
			if(HxOverrides.substr(s,i,1) == " ") xx += 40; else if(HxOverrides.substr(s,i,1) == "_") {
				xx = 60;
				yy += 180;
			} else {
				var bpoints = new typo.BePoints();
				var pp = bpoints.getLetterPoints(HxOverrides.substr(s,i,1),false,2,font,true);
				var ww;
				ww = (function($this) {
					var $r;
					var key = HxOverrides.substr(s,i,1);
					$r = font.widths.get(key);
					return $r;
				}(this)) * 2;
				var _g3 = 0;
				var _g2 = pp.length;
		...
		if(Main.letter == null) {
			Main.letter = new typo.Letter();
			Main._stage.addChild(Main.letter);
		}
		var rect = this._getRect(allPoints);
		Main.letter.x = window.innerWidth / 2 - rect.x - rect.width / 2;
		Main.letter.y = window.innerHeight / 2 - rect.y - rect.height / 2;
		if(Main.letter.x < 0) Main.letter.x = 0;
		if(Main.letter.y < 0) Main.letter.y = 0;
		Main.letter.init(allPoints);
	}
	,_getRect: function(allPoints) {
		var minX = Math.POSITIVE_INFINITY;
		var minY = Math.POSITIVE_INFINITY;
		var maxX = 0;
		var maxY = 0;
		var _g1 = 0;
		var _g = allPoints.length;
		while(_g1 < _g) {
			var i = _g1++;
			var p = allPoints[i];
	...
	,_onResize: function(e) {
		Main._canvas.width = window.innerWidth;
		Main._canvas.height = window.innerHeight;
		Main._stage.clear();
		Main.letter.start();
	}
};
...
typo.Be2d = function(p0,p1,ctrl) {
	...
	this._point1 = p1;
	this._control = ctrl;
	console.log(Std.string(this._point0) + " " + Std.string(this._point1) + " " + Std.string(this._control));
	this.setPoints(this._point0,this._point1,ctrl);
};
typo.Be2d.__name__ = true;
typo.Be2d.prototype = {
	f: function(t) {
		var tp = 1.0 - t;
		return new createjs.Point(this._point0.x * tp * tp + 2 * this._control.x * t * tp + this._point1.x * t * t,this._point0.y * tp * tp + 2 * this._control.y * t * tp + this._point1.y * t * t);
	}
	,diff: function(t) {
		return new createjs.Point(2 * (t * (this._point0.x + this._point1.x - 2 * this._control.x) - this._point0.x + this._control.x),2 * (t * (this._point0.y + this._point1.y - 2 * this._control.y) - this._point0.y + this._control.y));
	}
	,integral: function(t) {
	...
	,integralInit: function() {
		...
		console.log("integralInit " + ax + " " + kx + " " + this.XY);
		this.XY = kx * kx + ky * ky;
		...
		console.log(this.C + " " + this.CS + " " + this.CS2);
		this.INTG_0 = this.integralBezje(0.0);
		console.log(this.INTG_0);
	...
	,integralBezje: function(t) {
		...
		console.log("AA" + this.B + " " + t + " " + BTS + " " + this.XY + " " + this.CS);
...
typo.BePoints.prototype = {
	getLetterPoints: function(moji,isCentering,scale,letter,isLineSplit) {
		...
		while(_g < len) {
			...
			case "M":
				...
				this._count++;
				this._points[this._count] = [new createjs.Point(this._currentX,this._currentY)];
				break;
			case "L":
				if(isLineSplit) this.lineTo(s * (motif[i][1][0] + ox),s * (motif[i][1][1] + oy)); else {
					this._currentX = s * (motif[i][1][0] + ox);
					this._currentY = s * (motif[i][1][1] + oy);
					this._points[this._count].push(new createjs.Point(this._currentX,this._currentY));
				}
				break;
			case "C":
	...
	,lineTo: function(xx,yy) {
		...
		while(_g < numBunkatsu) {
			...
			var pp = new createjs.Point(this._currentX + rate * dx,this._currentY + rate * dy);
	...
	,curveTo: function(cx,cy,xx,yy) {
		var bezje = new typo.Be2d(new createjs.Point(this._currentX,this._currentY),new createjs.Point(xx,yy),new createjs.Point(cx,cy));
...
typo.Letter.__name__ = true;
typo.Letter.__super__ = createjs.Container;
typo.Letter.prototype = $extend(createjs.Container.prototype,{
	init: function(allPoints) {
		this._rad = Math.random() * 2 * Math.PI;
		this._count = 0;
		if(this.shape == null) {
			this.shape = new createjs.Shape();
			this.addChild(this.shape);
			this.shape.graphics.beginStroke(createjs.Graphics.getRGB(255,255,255,this.mojiAlpha)).setStrokeStyle(1,0,0,0,true);
		}
		this._allPoints = allPoints;
		var len = Math.floor(this._allPoints.length / 24);
		var _g = 0;
		while(_g < len) {
			var i = _g++;
			var idx1 = Math.floor(Math.random() * this._allPoints.length);
			var idx2 = Math.floor(Math.random() * this._allPoints.length);
			var tmp = this._allPoints[idx1];
			this._allPoints[idx1] = this._allPoints[idx2];
			this._allPoints[idx2] = tmp;
		}
		this.isColor = !this.isColor;
		if(this.isColor) this.mojiAlpha = 0.06; else this.mojiAlpha = 0.02;
		if(this.isColor) window.document.body.style.backgroundColor = "#888888"; else window.document.body.style.backgroundColor = "#ffffff";
	}
	,start: function() {
		this._count = 0;
	}
	,update: function() {
		if(this._count >= this._allPoints.length) return;
		var i = this._count;
		var _g1 = i;
		var _g = this._allPoints.length;
		while(_g1 < _g) {
			var j = _g1++;
			if(i != j) {
				var dx = this._allPoints[i].x - this._allPoints[j].x;
				var dy = this._allPoints[i].y - this._allPoints[j].y;
				var dist = Math.sqrt(dx * dx + dy * dy);
				if(dist < 100) {
					this.shape.graphics.moveTo(this._allPoints[i].x,this._allPoints[i].y);
					this.shape.graphics.lineTo(this._allPoints[j].x,this._allPoints[j].y);
				}
			}
		}
		this._count++;
		if(this.isColor) this.shape.compositeOperation = "darker"; else this.shape.compositeOperation = "normal";
	}
	,clear: function() {
		this.shape.graphics.clear();
		if(!this.isColor) this.shape.graphics.beginStroke(createjs.Graphics.getRGB(0,0,0,this.mojiAlpha)); else this.shape.graphics.beginStroke(createjs.Graphics.getRGB(Math.floor(128 + 128 * Math.sin(this._count / 200 + this._rad)),Math.floor(128 + 128 * Math.sin(this._count / 400 + this._rad)),Math.floor(128 + 128 * Math.sin(this._count / 600 + this._rad)),this.mojiAlpha));
	}
});
var $_, $fid = 0;
